...
public class Auction_extendMethod {
	...
	//	System.out.println("Auction_Extend Start: userNum: "+ users.size() +", serverNum: "+ servers.size());
	...
	public static double getAlgorithm1Infocom19(Random rad, ArrayList<EdgeUser> users, ArrayList<BaseStation> stations, ArrayList<PhysicalMachine> servers, Result tempResult, int[] baseStationDecision) {
		...
		double overallSystemCost = Result.getOverallSystemCostfeua(servers, users, tempResult);
		...
		//overall fairness
		double overallfairness = Result.getoverallfairness(servers, users, tempResult);
		tempResult.setfairness(overallfairness);
	...
	public static Result getAlgorithm3Infocom19(Random rad, ArrayList<EdgeUser> users, ArrayList<BaseStation> stations, ArrayList<PhysicalMachine> servers) {
		...
		sortEdgeServersDescendingByEnergy(servers);
	...
	public static void sortEdgeServersDescendingByEnergy(ArrayList<PhysicalMachine> servers) {
		for(int i=0;i<servers.size();i++) {
			for(int j=i+1;j<servers.size();j++) {
				if(servers.get(i).getEnergyCost()<servers.get(j).getEnergyCost()) {
					PhysicalMachine tempeEdgeServer = new PhysicalMachine(servers.get(i));
					servers.set(i, new PhysicalMachine(servers.get(j)));
					servers.set(j,new PhysicalMachine(tempeEdgeServer));
				}
			}
		}
	}
