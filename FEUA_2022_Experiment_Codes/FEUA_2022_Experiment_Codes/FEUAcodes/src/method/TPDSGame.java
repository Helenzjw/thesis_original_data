...
public class TPDSGame {
	public static Result GetBest(ArrayList<PhysicalMachine> machines, ArrayList<BaseStation> stations, ArrayList<EdgeUser> users) {
			...
			//System.out.println("TPDS2019Game Start: userNum: "+ users.size() +", serverNum: "+ machines.size());
		...
		public static ArrayList<Decision> getDecisionStrategyPhase1(ArrayList<PhysicalMachine> machines, ArrayList<BaseStation> stations, ArrayList<EdgeUser>users,Result result){
			...
			for(int i=0;i<users.size();i++) {
				...
				if(Decision.isUnallocated(result.getDecisions().get(i))) {
					...
					//maxBenefit = EdgeUser.getBenefitOfUsertpds(machines.get(result.getDecisions().get(i).getServer()).getnUsers(), users.get(i).getDataRate(), machines.get(result.getDecisions().get(i).getServer()));
					maxBenefit = EdgeUser.getBenefitOfUsertpds(machines.get(result.getDecisions().get(i).getServer()).getnUsers(), users.get(i), users.get(i).getDataRate(), machines.get(result.getDecisions().get(i).getServer()));
				}
					...
					for(int k=0;k<ConstNum.nChannel;k++) {
						...
						if(PhysicalMachine.remainingCapacitiesCheck(machines.get(users.get(i).getCoveredserver().get(j)), users.get(i))) {
							...
							if(users.get(i).getCoveredserver().get(j) == result.getDecisions().get(i).getServer()) {
								 benefit = EdgeUser.getBenefitOfUsertpds(machines.get(users.get(i).getCoveredserver().get(j)).getnUsers(), users.get(i), dataRate, machines.get(users.get(i).getCoveredserver().get(j)));
										
							}else {
								 benefit = EdgeUser.getBenefitOfUsertpds(machines.get(users.get(i).getCoveredserver().get(j)).getnUsers()+1, users.get(i), dataRate, machines.get(users.get(i).getCoveredserver().get(j)));
							}
